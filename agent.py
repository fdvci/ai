...
# Local imports
from nova_types import (
    Memory, Goal, Insight, PerformanceMetrics, MemoryType, 
    NovaError, APIError, MemoryError, ValidationError
)
...
class AsyncSafeCounter:
    """Async-safe counter for metrics"""
...
class NovaAgent:
    ...
    def __init__(self):
        ...
        self._memory_lock = None  # Will be initialized in async context
        ...
        self.exchange_count = None  # Will be AsyncSafeCounter
        self.context_window = []
        ...
        # Initialize these as None, will be set in async context
        self._memory_lock = None
        self._metrics_lock = None
        self.exchange_count = None
    
    async def initialize_async_components(self):
        """Initialize async components that need event loop"""
        self._memory_lock = asyncio.Lock()
        self._metrics_lock = asyncio.Lock()
        self.exchange_count = AsyncSafeCounter(0)
    ...
    def _initialize_components(self):
        ...
        try:
            ...
            logger.info("[OK] Vector memory initialized")
            
            # Load brain and state
            self.brain = self.load_brain()
            self.state = self.load_state()
            logger.info("[OK] Brain and state loaded")
            ...
            logger.info("[OK] Autonomy and self-modification initialized")
    ...
    def _initialize_openai_client(self):
        ...
        try:
            ...
            logger.info("[OK] OpenAI client initialized and tested")
...
Always strive to be helpful, accurate, and engaging while demonstrating genuine learning and growth."""
    ...
    async def save_brain(self):
        ...
        try:
            async with self._memory_lock:
                ...
                # Write to temporary file first (in thread pool)
                def write_brain():
                ...
                await loop.run_in_executor(None, write_brain)
    ...
    async def handle_realtime_query_async(self, user_input: str) -> Optional[str]:
        """Handle queries requiring real-time information with error handling (async version)"""
        try:
            ...
            # Perform web search in thread pool
            loop = asyncio.get_running_loop()
            search_results = await loop.run_in_executor(
                None, search_web, search_query, self.config.max_search_results
            )
    ...
    async def retrieve_relevant_memories(self, query: str, k: int = 5) -> List[Memory]:
    ...
    async def generate_contextual_response_async(self, user_input: str) -> str:
        """Generate response with full context awareness and error handling (async version)"""
        try:
            # Build comprehensive context
            relevant_memories = await self.retrieve_relevant_memories(user_input)
    ...
    async def chat(self, user_input: str, structured: bool = False) -> Union[str, Dict[str, Any]]:
        ...
        try:
            ...
            if user_input == "__autonomous__":
                ...
                loop = asyncio.get_running_loop()
                return await loop.run_in_executor(None, self.autonomy.meta_thought)
            
            # Update counters
            await self.exchange_count.increment()
            async with self._metrics_lock:
            ...
            try:
                async with self._memory_lock:
                    conv_memory = Memory(
                        ...
                        content=user_input,
                        metadata={
                            "exchange_count": await self.exchange_count.get(),
            ...
            if self.needs_realtime_info(user_input):
                search_result = await self.handle_realtime_query_async(user_input)
                if search_result:
                    response = search_result
            
            if not response:
                response = await self.generate_contextual_response_async(user_input)
                ...
                if patterns.get("areas_for_improvement"):
                    for area in patterns["areas_for_improvement"][:2]:  # Limit to 2 areas
                        try:
                            ...
                            loop = asyncio.get_running_loop()
                            search_results = await loop.run_in_executor(
                                None, search_web, f"how to improve {area} in AI conversation"
                            )
                            if search_results and search_results[0] != "No useful search results found.":
    ...
    def get_status_report(self) -> Dict[str, Any]:
        ...
        try:
            return {
                ...
                "agent_info": {
                    "speaker": self.speaker,
                    "exchange_count": self.exchange_count.get() if hasattr(self.exchange_count, 'get') else 0,
                    "total_memories": len(self.brain),
    ...
    def shutdown(self):
        ...
        try:
            # Note: These will be called from async context in main.py
